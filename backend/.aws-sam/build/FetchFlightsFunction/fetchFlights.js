...
exports.handler = async (event) => {
    try {
        ...
        console.log(
            `GlobalState: isActive=${isActive}, activeConnections=${activeConnections}`
        );
        ...
        if (!isActive || activeConnections === 0) {
            ...
            return {
                statusCode: 200,
                body: JSON.stringify({
                    message: "Fetch stopped - no active connections",
                }),
            };
        }
        ...
        await lambda.send(
            new InvokeCommand({
                FunctionName:
                    process.env.BROADCAST_FUNCTION_NAME || "broadcast",
        ...
        return {
            statusCode: 200,
            body: JSON.stringify({
                message: "Flugdaten gefetched und gebroadcasted",
            }),
        };
    } catch (error) {
